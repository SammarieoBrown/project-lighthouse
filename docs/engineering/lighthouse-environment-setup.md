...
**2. Direct connections are limited; there is a pooled endpoint.** The string in `.env` is the direct endpoint. Since the job queue is Postgres `SKIP LOCKED` (PRD §11.6), every worker holds a connection, and connection count is the thing that will strain first. The pooled endpoint is the same host with `-pooler` appended to the endpoint ID; confirm the exact hostname in the Neon console rather than trusting the commented line in `.env`. Workers should wake on `LISTEN/NOTIFY` rather than tight-polling — fewer wasted round trips, and it avoids holding Neon awake purely to ask whether anything happened.
