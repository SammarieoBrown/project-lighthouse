...
/* Districts and homes become GeoJSON point features carrying their counts, so
 * every paint decision below is an expression over real properties rather than
 * a colour baked in at export time. Change the severity rule and the map
 * follows without re-exporting the snapshot. */
export function dataSources(snapshot: Snapshot): Record<string, SourceSpecification> {
  ...
  return {
    ...
    "lh-cone": {
      type: "geojson",
      data: snapshot.cone
        ? { type: "Feature", properties: {}, geometry: snapshot.cone as GeoJSON.Geometry }
        : { type: "FeatureCollection", features: [] },
    },
    "lh-track": {
      type: "geojson",
      data: snapshot.track
        ? { type: "Feature", properties: {}, geometry: snapshot.track as GeoJSON.Geometry }
        : { type: "FeatureCollection", features: [] },
    },
    "lh-districts": {
      type: "geojson",
